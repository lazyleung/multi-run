...
function initMongo(mongoConfig, done){
    ...
    var usr = mongoConfig.usrName;
    var pwd = mongoConfig.pwd;
    ...
    g.mongoClient = new Db(mongoConfig.dbName, server, optionsWithEnableWriteAccess);

    openCollection(mongoConfig.collectionName, usr, pwd, done);
}

function openCollection(collectionName, usr, pwd, done){
    g.mongoClient.open(function(error, client) {
        if(!error){
            client.authenticate(usr, pwd, function(err, success) {
                onDbReady(err);
            });
        }
    });
